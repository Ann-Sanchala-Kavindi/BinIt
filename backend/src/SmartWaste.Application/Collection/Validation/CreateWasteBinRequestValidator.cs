...
/// <summary>
/// Validator for municipal roadside waste bin registration requests.
/// Enforces shape, bounds, and structural constraints per api-contract.md.
/// </summary>
public class CreateWasteBinRequestValidator : AbstractValidator<CreateWasteBinRequest>
{
    public CreateWasteBinRequestValidator()
    {
        RuleFor(x => x.BinCode)
            .NotEmpty().WithMessage("Bin code is required.")
            .Length(3, 50).WithMessage("Bin code must be between 3 and 50 characters.")
            .Matches(@"^[A-Za-z0-9-]+$").WithMessage("Bin code must contain only alphanumeric characters and hyphens.");
